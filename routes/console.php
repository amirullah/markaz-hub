...
/**
 * Sinkron manual semua koneksi Tokopedia/TikTok.
 */
Artisan::command('tokpedtiktok:sync {--catalog : ikut sinkron katalog produk}', function () {
    $conns = \App\Models\MarketplaceConnection::withoutGlobalScopes()
        ->where('platform', 'TIKTOKTOKO')->where('status', 'CONNECTED')->get();
    if ($conns->isEmpty()) {
        $this->info('Belum ada toko yang terhubung ke Tokopedia/TikTok.');
        return;
    }
    $sync = app(\App\Services\TokpedTikTok\TokpedTikTokSync::class);
    foreach ($conns as $c) {
        try {
            $r = $sync->syncStore($c);
            $this->info("Toko #{$c->store_id}: {$r['message']}");
...
})->purpose('Sinkron pesanan+settlement Tokopedia/TikTok untuk semua toko terhubung');
...
Schedule::command('tokpedtiktok:sync')->everyFourHours()->withoutOverlapping();
Schedule::command('tokpedtiktok:sync --catalog')->dailyAt('03:20')->withoutOverlapping();
